...
/*********************************************************************************/
/*     -------------------------------------------------------------------       */
/*                        MPEG-4 Mp3SampleEntry Class                          */
/*     -------------------------------------------------------------------       */
/*********************************************************************************/
/*
    This Mp3SampleEntry Class is used for visual streams.
    Actually, mp3 track does not have any codec specific info.
    But, we construct the codec specific info to get channel number and samplerate outside easier.
*/
...
// Stream-in ctor
Mp3SampleEntry::Mp3SampleEntry(MP4_FF_FILE *fp, uint32 size, uint32 type)
        : SampleEntry(fp, size, type)
{
    ...
    if (_success)
    {
		...
		LOGI("\n MP3 audio channel coount %d, _timeScale %d  +++++++++++ \n\n", _channelCount, _timeScale);


		/* NOTE: .mp3 atom has no ESD description, so pes is NULL */
        if (_success)
        {
			#if 0
